...
def load(item):
    ...
    return (prompt, image)
layer = 24
gpus = ["cuda:0","cuda:1","cuda:2","cuda:3","cuda:4","cuda:5","cuda:6"]
w = open("class_pairs.txt", 'r').read().splitlines() 
steering_vectors = []
for i in w:
    cls1 = i.split("@")[0]
    cls2 = i.split("@")[1]
    steering_vectors.append(torch.load(f"steering_vectors/{cls1}_to_{cls2}/diffmeans_layer{layer}.pt"))
models = []
...
np.save(f"diffmeans_l{layer}_steering_damage.npy",np.array(damage_values))
